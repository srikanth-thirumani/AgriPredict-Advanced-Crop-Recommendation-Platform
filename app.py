...
def get_weather_data(location):
    """
    Fetch weather data using OpenWeatherMap API by location name
    """
    base_url = "http://api.openweathermap.org/data/2.5/weather"
    params = {
        'q': location,
...
def index():
    ...
    # Get the OpenWeatherMap API key for the frontend to use
    # This is safe because API requests will be restricted by domain on the server
    openweather_api_key = os.environ.get('OPENWEATHER_API_KEY')
    return render_template('index.html', soil_types=soil_types, openweather_api_key=openweather_api_key)
